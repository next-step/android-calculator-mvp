...
internal class CalculatorMemoryTest {
    ...
    internal fun `완성된 수식이 주어질 때 계산 기록이 저장되어야 한다`() {
        //given
        calculatorMemory = CalculatorMemoryImpl()
    ...
    internal fun `저장된 계산 수식을 가져올 수 있어야 한다`() {
        //given
        val expression1 = CalculatorMemoryImpl.SAVE_FORMAT.format(Expression(listOf(3, Operator.Plus, 5)), 8)
        val expression2 = CalculatorMemoryImpl.SAVE_FORMAT.format(Expression(listOf(4, Operator.Plus, 2)), 6)
        calculatorMemory = CalculatorMemoryImpl(arrayListOf(expression1, expression2))
